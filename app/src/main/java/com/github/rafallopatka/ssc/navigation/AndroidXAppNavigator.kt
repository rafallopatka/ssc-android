...
@Suppress("UNUSED_PARAMETER", "MemberVisibilityCanBePrivate", "SpellCheckingInspection")
open class AndroidXAppNavigator(
    ...
    private fun copyStackToLocal() {
        ...
        val stackSize = fragmentManager.getBackStackEntryCount()
    ...
    protected fun activityForward(command: Forward) {
        val screen = command.screen as AndroidXAppScreen
    ...
    protected fun backTo(command: BackTo) {
        if (command.screen == null) {
            ...
            if (index != -1) {
                ...
                backToUnexisting(command.screen as AndroidXAppScreen)
            }
        }
    }
    ...
    private fun checkAndStartActivity(screen: AndroidXAppScreen, activityIntent: Intent, options: Bundle?) {
     ...
     */
    protected fun unexistingActivity(screen: AndroidXAppScreen, activityIntent: Intent) {
     ...
     */
    protected fun createFragment(screen: AndroidXAppScreen): Fragment? {
     ...
     * @param screen screen
     */
    protected fun backToUnexisting(screen: AndroidXAppScreen) {
        backToRoot()
    }

    protected fun errorWhileCreatingScreen(screen: AndroidXAppScreen) {
